...
def partition(array, lo, hi)
  ...
  (lo..hi-1).each_with_index do |j|
    if array[j] <= pivot
      # swap A[i] with A[j]
      puts "loop1:" + array.to_s
      array[i], array[j] = array[j], array[i]
      puts "loop2:" + array.to_s
      i = i + 1
    end
  end
  # swap A[i] with A[hi]
  puts "end 1" + array.to_s
  array[i], array[hi] = array[hi], array[i]
  puts "end 2" + array.to_s
  return i
end

test_array_1 = [72, 5, 12, 1, 6]
...
# solution
binding.pry
a = quicksort(test_array_1, 0, test_array_1.length - 1).to_s
puts '============'
puts "Sorted Array:"
puts a
